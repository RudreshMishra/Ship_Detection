...
def main_sentinel_2(input_directory,output_directory):
    ...
    for x0 in range(0, width, chopsize):
       for y0 in range(0, height, chopsize):
          ...
          img.crop(box).save(file_name+'/ChopedImages/ChopedImages/%s.%s.x%05d.y%05d.jpg' % (inputPath[:19],(time.localtime().tm_hour,time.localtime().tm_min), x0, y0))
    p_image.load_model(inputPath)
